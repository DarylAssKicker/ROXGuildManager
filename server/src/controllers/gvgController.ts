...
class GVGController {
   ...
   */
  async deleteGVGImage(req: Request, res: Response) {
    try {
      if (!req.user) {
        return res.status(401).json({
          success: false,
      ...
      if (process.env.UPLOAD_PATH) {
        imagePath = path.join(process.env.UPLOAD_PATH, 'GVG', date, decodedFilename);
      } else if (process.env.NODE_ENV === 'production') {
        imagePath = path.join(process.cwd(), 'client/public/images/GVG', date, decodedFilename);
      } else {
        imagePath = path.join(__dirname, '../../../client/public/images/GVG', date, decodedFilename);
      }
      ...
      console.error('Failed to delete GVG image:', error);
      return res.status(500).json({
        success: false,
        error: 'Internal server error',
        message: error instanceof Error ? error.message : 'Unknown error'
